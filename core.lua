...
function SB:build_full_name(name, server, realm)
  return string.format("%s-%s-%s", name, server, realm)
end

function SB:process_provider(l)
  -- Takes the given case data for a single provider and adds
  -- it to the database.
  for server, server_dict in pairs(l.realm_data) do
    for realm, realm_dict in pairs(server_dict) do
      ...
      for case_index, case_data in pairs(realm_dict) do
        ...
        case_data.server = server
        case_data.realm = realm
        case_data.provider = l.provider
        if case_data.name then
          case_data.full_name = SB:build_full_name(case_data.name, server, realm)
        end
  ...
  if case_data.players then
    for _, player_info in pairs(case_data.players) do
      if player_info.name then
        player_info.full_name = SB:build_full_name(player_info.name, case_data.server, case_data.realm)
      end
...
function SB:is_unit_eligible(unit_token)
  ...
  return self.db.realm.is_crossfaction or self.player_faction == UnitFactionGroup(unit_token)
end
...
function SB:check_unit(unit_token, unit_guid, scan_context)
  ...
  local full_name = self:build_full_name(name, self.db.global.server_name, realm)
  ...
  local shouldStop = not guid_match and (
    conf.require_guid_match
    or not self.name_to_incident_table[full_name]
  )
...
function SB:construct_chat_strings()
  ...
  local note_icon = "|TInterface\\Icons\\Inv_misc_note_02:16:16:0:-16:64:64:4:60:4:60|t"
  local diamond = "{rt3}"
...
function SB:GROUP_ROSTER_UPDATE()
  return self:OnRosterUpdate()
end

function SB:RAID_ROSTER_UPDATE()
  return self:OnRosterUpdate()
end

function SB:PARTY_MEMBERS_CHANGED()
  return self:OnRosterUpdate()
end
...
function SB:PARTY_INVITE_REQUEST(
  ...
  self:check_unit(nil, UnitGUID(name), "invite_confirmation")
end
